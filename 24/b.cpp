...
int main() {
    ...
    auto power = [&] (auto& g) {
        return g.size * (g.damage + (!g.infection ? boost : 0));
    };
    ...
    auto count_all = [&] {
        return accumulate(ALL(groups), 0, [] (int sum, auto& g) { return sum + g.size; });
    };
    while (true) {
        int prev_count = count_all() + 1;
        while (count(true) > 0 && count(false) > 0 && count_all() < prev_count) {
            prev_count = count_all();
        ...
        int immune_count = count(false);
        if (immune_count > 0 && count(true) == 0) {
            cout << immune_count << endl;
            return 0;
        }
        ++boost;
        groups = backup;
